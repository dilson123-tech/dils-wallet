# Aurea Gold Wallet — Asaas Sandbox First Customer HTTP Runtime Enable Contract V1

Milestone: `v0.2.61-wallet-asaas-sandbox-first-customer-http-runtime-enable-contract`

## Purpose

This milestone adds the runtime enable contract for the future first Asaas Sandbox `POST /customers` HTTP call.

The contract is a non-executing safety boundary. It can recognize the required runtime enable phrase and validate the previous explicit enable preflight, but it does not enable the adapter, does not enable HTTP execution and does not execute any Asaas request.

## Scope

The runtime enable contract is built on top of:
...
## Required runtime enable phrase

`CONFIRMO CONTRATO DE HABILITACAO RUNTIME ASAAS SANDBOX, SEM PRODUCAO E SEM DINHEIRO REAL.`
...
Even when the manual authorization phrase, explicit enable phrase and runtime enable phrase are all valid, this milestone keeps:

- `runtime_enable_contract_valid`: `true`
- `runtime_enable_allows_adapter_enablement`: `false`
- `runtime_enable_allows_http_execution`: `false`
...
`v0.2.62-wallet-asaas-sandbox-first-customer-http-runtime-switch-guard`
