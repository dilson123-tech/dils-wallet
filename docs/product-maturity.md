...
- v0.2.67-wallet-asaas-sandbox-first-customer-http-adapter-boundary-final-contract: final adapter boundary contract for the future first Asaas Sandbox POST /customers call, defining the allowed future sanitized execution handler caller and requiring the full safety chain before any future adapter can be considered, while keeping the current boundary contract-only, adapter unimplemented, adapter disabled, HTTP blocked, production blocked, real money disabled, raw provider payload blocked, request body hidden, raw errors hidden and stacktraces blocked.

- v0.2.68-wallet-asaas-sandbox-first-customer-http-final-manual-execution-runbook-readiness-gate: final manual execution runbook readiness gate for the future first Asaas Sandbox POST /customers call, confirming the full non-executing safety chain is ready for manual operator review before any future Sandbox attempt while keeping the current gate review-only, adapter unimplemented, adapter disabled, HTTP blocked, production blocked, real money disabled, raw provider payload blocked, request body hidden, raw errors hidden and stacktraces blocked.

- v0.2.69-wallet-asaas-sandbox-first-customer-http-first-controlled-sandbox-attempt-preparation: first controlled Sandbox attempt preparation contract for the future first Asaas Sandbox POST /customers call, requiring the final manual execution runbook readiness gate, manual operator review, Sandbox-only configuration, environment-loaded secrets, sanitized success/error envelopes, redacted logs, single controlled attempt policy, no retry loop and manual stop on unexpected response while keeping the current preparation non-executing, adapter unimplemented, adapter disabled, HTTP blocked, production blocked, real money disabled, raw provider payload storage blocked, request body logging blocked and stacktraces blocked.
